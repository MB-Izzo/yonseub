...
export const MainLayout = ({ children, title, ...props }: MainLayoutProps) => {
    ...
    return (
        <div className='min-h-full grid-lines'>
            <nav className='bg-gray-200 border border-black border-2 border-bottom rounded-b-4xl py-6'>
                <div className='flex justify-between w-6xl mx-auto'>
                    <div>
                        Logo
                    </div>
                    <div className='flex justify-between gap-8'>
                        <a href={auth.user ? '/app' : '/'} className={url === '/' || url === '/app' ? 'text-black font-black' : 'text-gray-600 hover:text-black'}>Practice</a>
                        <a href={'/my-exercises'} className={url === '/my-exercises' ? 'text-black font-black' : 'text-gray-600 hover:text-black'}>Your corrections</a>
                        <a href={'/words'} className={url === '/words' ? 'text-black font-black' : 'text-gray-600 hover:text-black'}>Words</a>
                    </div>
                    <div className='flex justify-between gap-8'>
                        <a href={'/my-exercises'} className={url === '/my-exercises' ? 'text-black font-black' : 'text-gray-600'}>Profile</a>
                        <Form method='POST' action={route('logout')} >
                            <button className='text-gray-600 hover:text-black hover:cursor-pointer'>Logout</button>
                        </Form>
                    </div>
                </div>
            </nav >
